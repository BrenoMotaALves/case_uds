"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CardsController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const create_card_usecase_1 = require("../application/create-card.usecase");
const delete_card_usecase_1 = require("../application/delete-card.usecase");
const move_card_usecase_1 = require("../application/move-card.usecase");
const update_card_usecase_1 = require("../application/update-card.usecase");
const cards_dto_1 = require("./cards.dto");
let CardsController = class CardsController {
    constructor(createCardUseCase, updateCardUseCase, deleteCardUseCase, moveCardUseCase) {
        this.createCardUseCase = createCardUseCase;
        this.updateCardUseCase = updateCardUseCase;
        this.deleteCardUseCase = deleteCardUseCase;
        this.moveCardUseCase = moveCardUseCase;
    }
    async create(columnId, body) {
        return this.createCardUseCase.execute({
            columnId,
            title: body.title,
            description: body.description
        });
    }
    async update(id, body) {
        return this.updateCardUseCase.execute(id, {
            title: body.title,
            description: body.description
        });
    }
    async move(id, body) {
        return this.moveCardUseCase.execute(id, body.newColumnId);
    }
    async delete(id) {
        await this.deleteCardUseCase.execute(id);
    }
};
exports.CardsController = CardsController;
__decorate([
    (0, common_1.Post)('columns/:columnId/cards'),
    (0, common_1.HttpCode)(201),
    (0, swagger_1.ApiOperation)({ summary: 'Create a card in a column' }),
    (0, swagger_1.ApiParam)({ name: 'columnId', example: 'c2e6d93b-5c35-4c3f-8a44-2f0b7a987d9d' }),
    (0, swagger_1.ApiResponse)({ status: 201, description: 'Card created' }),
    (0, swagger_1.ApiResponse)({ status: 404, description: 'Column not found' }),
    __param(0, (0, common_1.Param)('columnId')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, cards_dto_1.CreateCardDto]),
    __metadata("design:returntype", Promise)
], CardsController.prototype, "create", null);
__decorate([
    (0, common_1.Put)('cards/:id'),
    (0, swagger_1.ApiOperation)({ summary: 'Update a card' }),
    (0, swagger_1.ApiParam)({ name: 'id', example: 'd3f8fbb5-1d52-44be-9b1f-48ad94aefb8b' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Card updated' }),
    (0, swagger_1.ApiResponse)({ status: 404, description: 'Card not found' }),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, cards_dto_1.UpdateCardDto]),
    __metadata("design:returntype", Promise)
], CardsController.prototype, "update", null);
__decorate([
    (0, common_1.Patch)('cards/:id/move'),
    (0, swagger_1.ApiOperation)({ summary: 'Move a card to another column' }),
    (0, swagger_1.ApiParam)({ name: 'id', example: 'd3f8fbb5-1d52-44be-9b1f-48ad94aefb8b' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Card moved' }),
    (0, swagger_1.ApiResponse)({ status: 404, description: 'Card or column not found' }),
    (0, swagger_1.ApiResponse)({ status: 422, description: 'Invalid move' }),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, cards_dto_1.MoveCardDto]),
    __metadata("design:returntype", Promise)
], CardsController.prototype, "move", null);
__decorate([
    (0, common_1.Delete)('cards/:id'),
    (0, common_1.HttpCode)(204),
    (0, swagger_1.ApiOperation)({ summary: 'Delete a card' }),
    (0, swagger_1.ApiParam)({ name: 'id', example: 'd3f8fbb5-1d52-44be-9b1f-48ad94aefb8b' }),
    (0, swagger_1.ApiResponse)({ status: 204, description: 'Card deleted' }),
    (0, swagger_1.ApiResponse)({ status: 404, description: 'Card not found' }),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], CardsController.prototype, "delete", null);
exports.CardsController = CardsController = __decorate([
    (0, swagger_1.ApiTags)('cards'),
    (0, common_1.Controller)(),
    __metadata("design:paramtypes", [create_card_usecase_1.CreateCardUseCase,
        update_card_usecase_1.UpdateCardUseCase,
        delete_card_usecase_1.DeleteCardUseCase,
        move_card_usecase_1.MoveCardUseCase])
], CardsController);
//# sourceMappingURL=cards.controller.js.map